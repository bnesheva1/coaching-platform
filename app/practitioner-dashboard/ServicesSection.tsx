  ...
  return (
    <li style={{ marginBottom: "1rem" }}>
      ...
      <form action={setServiceActive.bind(null, service.id, !service.is_active)} style={{ display: "inline" }}>
        <button type="submit">{service.is_active ? "Hide" : "Show"}</button>
      </form>{" "}
      <form
  ...
  return (
    <section style={{ marginTop: "2rem", maxWidth: 400 }}>
      ...
      {services.length > 0 ? (
        ...
        <p style={{ color: "#666" }}>You haven&apos;t added any services yet.</p>
      )}
      ...
      >
        ...
        {state?.error && <p style={{ color: "crimson" }}>{state.error}</p>}
        {state?.success && <p style={{ color: "green" }}>Service added!</p>}
        <button type="submit" disabled={pending}>
