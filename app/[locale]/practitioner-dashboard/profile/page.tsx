...
export default async function ProfilePage({
  ...
  const [{ data: profile }, { data: practitionerProfile }, { data: services }, { data: reviews }, { data: connectStatusRaw }] =
    await Promise.all([
      ...
      supabase
        ...
        .select("bio, headline, location, specialties, topics, avatar_url, banner_url, username, timezone")
        .eq("id", userId)
        .single(),
  ...
  return (
    <main style={{ padding: "var(--space-8) 0" }} className={styles.content}>
      {/* No ContentContainer — DashboardShell already bounds/pads the
          sidebar+content row; see the other dashboard pages' identical
          note. No maxWidth cap either (unlike their 500px reading
          column) — this is a visual profile layout (banner/avatar/
          grid), not a form, and wants to use the width it's given. The
          negative right margin (page.module.css) cancels the shell's
          own right padding so that width isn't squeezed on this one
          page — left padding (separation from the sidebar) stays. */}
